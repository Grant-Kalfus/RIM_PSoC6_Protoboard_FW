...
void enable_translator(byte state, byte enable_id)
{
    switch(enable_id)
    {
        case 0:
            ...
            break;
        case 1:
            Cy_GPIO_Write(m1_enable_PORT, m1_enable_NUM, state);
            break;
        case 2:
            Cy_GPIO_Write(m2_enable_PORT, m2_enable_NUM, state);
            break;
        case 3:
...
// Coped from dSPIN_Move of SparkFun Arduino libraries for L6470
...
//Adapted from dSPIN_Xfer of SparkFun Arduino libraries for L6470
byte transfer(byte data, byte enable_id) {
    enable_translator(0, enable_id);
    _bufferTx[0] = data;
    (void) Cy_SCB_SPI_Transfer(SPI_HW, _bufferTx, _bufferRx, 1, &SPI_context);
...
//Copied from dSPIN_ParamHandler of Sparkfun arduino librarys for L6470
uint param_handler(byte param, uint value, byte enable_id )
{
  ...
  switch (param)
  {
    ...
    case RIM_STATUS:  // STATUS is a read-only register
...
//Adapted from dSPIN_Param of SparkFun Arduino libraries for L6470
